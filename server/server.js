...
function requireAdmin(req, res, next) {
  if (!isAdminRequest(req)) return res.status(401).json({ error: 'Not logged in' });
  next();
}
...
app.post('/api/players/claim', (req, res) => {
  let { name } = req.body || {};
  name = String(name || '').trim().replace(/\s+/g, ' ').slice(0, 20);
  if (name.length < 2) return res.status(400).json({ error: 'Name must be at least 2 characters' });
  const nameLower = name.toLowerCase();

  const existing = db.prepare('SELECT id FROM players WHERE name_lower = ?').get(nameLower);
  if (existing) return res.status(409).json({ error: 'That name is already taken. Pick another.' });

  const token = newToken();
  try {
    const info = db.prepare('INSERT INTO players (name, name_lower, token) VALUES (?, ?, ?)').run(name, nameLower, token);
...
function tournamentSummary(t) {
  const count = db.prepare('SELECT COUNT(*) AS n FROM participants WHERE tournament_id = ?').get(t.id).n;
  const host = t.host_player_id ? db.prepare('SELECT name FROM players WHERE id = ?').get(t.host_player_id) : null;
  return { ...t, participant_count: count, host_name: host ? host.name : null };
}

app.get('/api/tournaments', (req, res) => {
  const rows = db.prepare('SELECT * FROM tournaments ORDER BY datetime(created_at) DESC').all();
  res.json(rows.map(tournamentSummary));
});
...
app.get('/api/tournaments/:id', (req, res) => {
  ...
  res.json({ ...tournamentSummary(t), participants, matches });
});

app.get('/api/tournaments/:id/me', requirePlayer, (req, res) => {
  const row = db.prepare('SELECT id FROM participants WHERE tournament_id = ? AND player_id = ?')
    .get(req.params.id, req.player.id);
...
// ===================== SIGNUP =====================
app.post('/api/tournaments/:id/signup', requirePlayer, (req, res) => {
  const t = db.prepare('SELECT * FROM tournaments WHERE id = ?').get(req.params.id);
  ...
  const already = db.prepare('SELECT id FROM participants WHERE tournament_id = ? AND player_id = ?').get(t.id, req.player.id);
  ...
  const info = db.prepare('INSERT INTO participants (tournament_id, player_id, contact) VALUES (?, ?, ?)')
    .run(t.id, req.player.id, contact);
...
app.post('/api/admin/tournaments', requireAdmin, (req, res) => {
  ...
  `).run(fields.name, fields.game, fields.description, fields.max_participants, fields.starts_at);

  res.json({ id: info.lastInsertRowid });
});

// Community tournaments — any player with a claimed name.
app.post('/api/tournaments', requirePlayer, (req, res) => {
  let fields;
  try { fields = buildTournamentFromBody(req.body); }
  catch (err) { return res.status(400).json({ error: err.message }); }
  ...
  `).run(fields.name, fields.game, fields.description, fields.max_participants, fields.starts_at, req.player.id);

  res.json({ id: info.lastInsertRowid });
});

// ===================== TOURNAMENT MANAGEMENT (admin OR the community host who created it) =====================
function loadManageableTournament(req, res) {
  const t = db.prepare('SELECT * FROM tournaments WHERE id = ?').get(req.params.id);
...
app.patch('/api/tournaments/:id', (req, res) => {
  const t = loadManageableTournament(req, res);
  if (!t) return;
...
app.post('/api/matches/:matchId/winner', (req, res) => {
  ...
  try {
    setMatchWinner(match.id, (req.body || {}).winner_id);
  } catch (err) {
    return res.status(400).json({ error: err.message });
  }
...
app.post('/api/admin/login', (req, res) => {
  ...
  const token = newToken();
  db.prepare('INSERT INTO admin_sessions (token, expires_at) VALUES (?, ?)').run(token, Date.now() + SESSION_MS);
...
io.on('connection', socket => {
  ...
  socket.on('chat:send', ({ tournamentId, token, message }) => {
    ...
    db.prepare('INSERT INTO chat_messages (tournament_id, display_name, message) VALUES (?, ?, ?)')
      .run(tournamentId, player.name, text);
